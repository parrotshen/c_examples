...
#define rotl _generic_rotl32
#define rotr _generic_rotr32

#define uint32_in(x) le32_to_cpu(*(const uint32 *)(x))
#define uint32_out(to, from) (*(uint32 *)(to) = cpu_to_le32(from))
...
#define loop4(i)                                    \
{   t = rotr(t,  8); t = ls_box(t) ^ _rcoTable[i];    \
...
static uint32 _rcoTable[10] = {
        0x00000001, 0x00000002, 0x00000004, 0x00000008, 0x00000010, 0x00000020, 0x00000040, 0x00000080, 
        0x0000001b, 0x00000036, 
};

static uint32 _ftTable[4][256] = {
    {
...
/*
 * #define _byte(x, nr) ((unsigned char)((x) >> (nr*8))) 
 */
static inline uint8
